 ...
 */
__ASM uint32_t  __get_FPSCR(void)
{
  mrs r0, fpscr
  bx lr
}
 ...
 */
__ASM void __set_FPSCR(uint32_t fpscr)
{
  msr fpscr, r0
  bx lr
}
...
/**
 * @brief  Return the FPSCR
 *
 * @param  none
 * @return uint32_t FPSCR
 *
 * Return the content of the FPSCR
 */
uint32_t __get_FPSCR(void)
{
  uint32_t result=0;
  
  __ASM volatile ("FMRX %0, fpscr" : "=r" (result) );
  return(result);
}
...
{
  __ASM volatile ("FMXR fpscr, %0" : : "r" (value) );
}
